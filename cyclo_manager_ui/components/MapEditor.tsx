...
import { getPgmFiles, getPgmImage, savePgmImage } from "@/lib/api";
import type { PgmFileInfo, PgmImageResponse } from "@/types/api";
...
type UseMapEditorOptions = {
  open: boolean;
  mapName: string;
  onMessage: (message: string) => void;
};

function decodePgmPixels(image: PgmImageResponse): Uint8Array {
...
function encodePgmPixels(pixels: Uint8Array): string {
  let binary = "";
...
function pgmPixelsToGrid(image: PgmImageResponse, pixels: Uint8Array): MapEditorGrid {
  const data = new Array<number>(image.width * image.height);
...
function mapPointToPgmPixel(image: PgmImageResponse, x: number, y: number) {
...
export function useMapEditor({ open, mapName, onMessage }: UseMapEditorOptions) {
  ...
  const [image, setImage] = useState<PgmImageResponse | null>(null);
  ...
  useEffect(() => {
    ...
    getPgmFiles(CONTAINER)
      .then((response) => {
        ...
        setSelectedPath((current) => current || preferred?.path || response.files[0]?.path || "");
        if (!response.files.length) onMessage("No PGM files found");
      })
      .catch((error) => {
        if (!cancelled) onMessage(error instanceof Error ? error.message : "Failed to list PGM files");
      })
      .finally(() => {
  ...
  useEffect(() => {
    ...
    getPgmImage(CONTAINER, selectedPath)
      ...
      .catch((error) => {
        if (!cancelled) onMessage(error instanceof Error ? error.message : "Failed to load PGM file");
      })
      .finally(() => {
        if (!cancelled) setBusy(false);
      });
  ...
  const editAtMapPoint = useCallback((x: number, y: number) => {
    if (!open || !image || !pixels || busy || tool === "view") return;
    const pixel = mapPointToPgmPixel(image, x, y);
    if (!pixel) return;
    const { pixelX, pixelY } = pixel;
    const nextPixels = paintPgmPixels(
      pixels,
      image.width,
      image.height,
      pixelX,
    ...
    for (let index = 0; index < pixels.length; index += 1) {
      if (pixels[index] !== nextPixels[index]) editedPixels += 1;
    }
    if (editedPixels === 0) {
  ...
  const save = useCallback(() => {
    if (!image || !pixels || !dirty || busy) return;
    setBusy(true);
    savePgmImage(
      CONTAINER,
      image.path,
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      <select
        ...
        className="h-8 min-w-64 px-2 border text-sm"
        style={{
      ...
      <button
        ...
        className="h-8 w-8 border inline-flex items-center justify-center disabled:opacity-50"
        title="View"
        aria-label="View"
        style={{
          color: tool === "view"
      ...
      {(["erase_black", "draw_black"] as const).map((value) => (
        <button
          key={value}
          type="button"
          disabled={busy}
