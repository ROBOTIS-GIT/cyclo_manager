...
- **Jog** (`/jog`): Enable/stop controls, joystick or keyboard base input, and joint buttons that move continuously while pressed and stop on release. The server selects the running robot profile; joint metadata comes from URDF and ROS feedback. See [Jog](../docs/jog.md).
