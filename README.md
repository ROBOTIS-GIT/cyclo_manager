...
- **Launch arguments in the UI**: Gear icon on the System page opens a popup to edit bringup args per service. Follower bringup supports robot models **SG2 / BG2 / SH5 / BH5 / Mobile** (separate defaults and `localStorage` per model). **Init Position File** is a dropdown: robot-specific default YAML, **`pack_position.yaml`**, or **Custom input** (free-form filename). Settings are persisted per container in the browser.
