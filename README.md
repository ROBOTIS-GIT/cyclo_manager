...
- **In-browser terminals**: WebSocket terminal into running containers (`/terminal/{name}/ws`), with persistent **bash** sessions (direct PTY + server-side output buffer + replay on reconnect) so shells survive UI navigation. Multi-tab terminals, process list with kill, and per-container **bashrc** editing from the Docker page.
...
│  │  /{container}/bashrc  /version  /terminal/{name}/ws      ││
│  │  /ws/...                                                 ││
│  └──────────────────────────────────────────────────────────┘│
│       │                   │                    │             │
...
│   │                           # docker, terminal, ros2, websocket, version
│   ├── terminal_session_manager.py  # PTY session lifecycle (drain, replay)
...
| Terminal | `WebSocket /terminal/{name}/ws` | Interactive terminal (`session_id` query optional); PTY + replay |
| | `DELETE /terminal/{name}/{session_id}` | Close terminal session |
